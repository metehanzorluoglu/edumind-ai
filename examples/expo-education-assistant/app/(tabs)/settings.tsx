...
function providerLabel(provider: string | null): string {
  // null means a local email/password account — never a genuinely
  // "unknown" provider (the backend only omits this field for that one
  // case), so this reads as a real answer rather than a shrug.
  ...
  if (uri) {
    return (
      <Image
        ...
        style={[
          styles.avatarImage,
          { width: size, height: size, borderRadius: size / 2, backgroundColor: theme.cardPressed },
        ]}
      />
    );
  }
    ...
    >
      <Text
        style={[
          ...
          { fontSize: size * 0.4, fontFamily: theme.fonts.display },
        ]}
      >
        {initial}
      </Text>
...
// InfoDialog itself now lives in components/ui/InfoDialog.tsx — shared
// with the login page's legal footer (see lib/legalContent.ts) so Privacy/
// Terms copy can't drift between the two surfaces.
